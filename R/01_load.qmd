...
dir.create(here("presentation_data"))
```
...
saveRDS(count_data,
        file = here("presentation_data/01_count_data.rds"))
```

```{r}

saveRDS(clinical_data, 
        file = here("presentation_data/01_clinical_data.rds"))
```

```{r}

saveRDS(ensembl_to_gene_symbol, 
        file = here("presentation_data/01_gene_ensembl.rds"))
```
